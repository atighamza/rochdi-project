...
const socket = io("https://rochdi-project-back.onrender.com"); // Adjust if backend is hosted elsewhere
...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    api
      .get(`/messages/${user._id}/${selectedFriend._id}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((res) => {
  ...
  return (
    <Box sx={{ display: "flex", height: "100vh" }}>
      {/* Friends List */}
      <Box
        sx={{ width: 300, borderRight: "1px solid #ccc", overflowY: "auto" }}
      >
        <Box
          display="flex"
